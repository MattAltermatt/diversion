...
describe('boxcar2d schema additions', () => {
  ...
  it('defaults: time mode, dunes terrain, light rubble, gentle roughness, infinite track', () => {
    expect(d.mode).toBe('time')
    expect(d.terrainType).toBe('dunes')
    expect(d.rubbleDensity).toBe(1) // on, but only past the 100 m launch gap (index.ts)
