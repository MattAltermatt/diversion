...
// (Mirrors flock-vs-hunter/spatialHash.ts — generic, no diversion-specific state.)
export class SpatialHash {
   ...
   *  If `alive` is given, dead particles are excluded (so queries only see live ones). */
  ...
  /** Append up to `cap` neighbour indices of particle `i` within `r` into `out`
   *  (caller clears it). Distances are minimum-image; the 3×3 cell block wraps
   *  toroidally. */
  neighborsWithin(
