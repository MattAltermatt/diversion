...
export const pottsSchema = z.object({
  ...
  states: z.number().int().min(4).max(40).default(20)
    .meta({ section: 'Rules', ui: 'slider', min: 4, max: 40, step: 1, label: 'Orientations',
            help: 'How many distinct grain orientations (colours) exist. More = a richer stained-'
                + 'glass mosaic with more grains before neighbours share a colour and merge.' }),
  ...
  boundaryColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).default('#080810')
    .meta({ section: 'Color', ui: 'color', label: 'Wall colour',
            help: 'Colour of the grain-boundary outline (used only when Grain boundaries is on).' }),
  ...
  palette: z.object({
    hueStart: z.number().min(0).max(360).default(0)
      .meta({ ui: 'slider', min: 0, max: 360, step: 1, label: 'Hue start',
              help: 'Where the grain-colour ring begins on the hue wheel (degrees).' }),
    hueSpan: z.number().min(0).max(360).default(340)
      .meta({ ui: 'slider', min: 0, max: 360, step: 1, label: 'Hue span',
              help: 'How much of the hue wheel the grain colours cover. 360 = full rainbow; narrow = '
