...
export const swarmalatorsSchema = z.object({
  ...
  K: z.number().min(-1).max(1).default(-0.75)
    .meta({ section: 'Coupling', ui: 'slider', min: -1, max: 1, step: 0.05, label: 'Phase sync (K)',
            help: 'How strongly neighbours pull each other into the SAME phase. K>0 syncs everything to one colour; the interesting swarming states are at K≤0, where space and phase fight and the ring comes alive.' }),
