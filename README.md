...
- **A framework** that gives every diversion two screens for free:
  - **Config screen** — controls beside a live preview on a wide screen; on a narrow one the preview pins to the top and the controls flow beneath it. The piece's own description sits under its title, so a shared link explains what you're looking at without a trip back to the gallery.
...
- **`meta.ts`** — the identity (`id`, `title`, `description`, `kind`). Eagerly loaded, so the gallery can lay out all 139 tiles on the first paint.
